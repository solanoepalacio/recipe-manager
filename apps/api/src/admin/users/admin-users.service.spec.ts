...
describe('AdminUsersService', () => {
  ...
  describe('create', () => {
    it('hashes password when provided', async () => {
      ...
      const result = await (service as any).create({ householdId: 'h1', name: 'Bob', email: 'bob@ex.com', password: 'secret123', gender: 'other', dateOfBirth: '2000-01-01' });
    ...
    it('creates user with null passwordHash when no password provided', async () => {
      ...
      await (service as any).create({ householdId: 'h1', name: 'Carol', userType: 'kid', dateOfBirth: '2010-01-01' });
